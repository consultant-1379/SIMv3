...
# check the user before anything
if [[ $(id -nu) != "dcuser" ]];then
   echo "Not dcuser user, exiting.."
   exit 1
fi
...
$JAVA -Duser.dir=$APP_HOME -Dapp.home=$APP_HOME -Dlog4j.configurationFile=file:$ETCDIR/log4j2.xml -Dconfig=$CONFIG_DEF -cp "$LIBDIR/*:$EXTLIBDIR/*:$POLICYDIR/*:$PROTOCOLDIR/*" com.ericsson.sim.engine.Main "$@" &

echo "Checking process ..."
#check pid
PID=$(ps -ef | grep java | grep $APP_NAME | grep -v grep | awk '{print $2}')
#if it exists, check if it keeps running
if [[ -n $PID ]]; then
  sleep 5
  PID=$(ps -ef | grep java | grep $APP_NAME | grep -v grep | awk '{print $2}')
  if [[ -z $PID ]]; then
    echo "$APP_NAME seemed to have stopped. Check logs for more information"
    exit 1
  fi
else
  echo "$APP_NAME seemed to have stopped. Check logs for more information"
